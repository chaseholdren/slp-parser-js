...
export function generateConversions(game: SlippiGame): ConversionType[] {
  ...
  iterateFramesInOrder(
    ...
    (indices, frame) => {
      ...
      const prevOpponentFrame: PostFrameUpdateType = lodash.get(
        frames,
...
function addOpeningTypeToConversions(game: SlippiGame, conversions: ConversionType[]) {
  const conversionsByPlayerIndex = lodash.groupBy(conversions, 'playerIndex');
  const keyedConversions = lodash.mapValues(conversionsByPlayerIndex, (playerConversions) =>
    lodash.keyBy(playerConversions, 'startFrame'),
  );
  ...
  iterateFramesInOrder(
    ...
    (indices, frame) => {
      ...
      if (lodash.get(state, ['opponentConversion', 'endFrame']) === frameNum) {
        state.opponentConversion = null;
      }

      // Get opponent conversion. Add to state if exists for this frame
      const opponentConversion = lodash.get(keyedConversions, [indices.opponentIndex, frameNum]);
      if (opponentConversion) {
        state.opponentConversion = opponentConversion;
      }

      const playerConversion = lodash.get(keyedConversions, [indices.playerIndex, frameNum]);
      if (!playerConversion) {
